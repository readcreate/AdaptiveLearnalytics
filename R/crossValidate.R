...
#' if (!require(Metrics)) install.packages('Metrics')
#' library(Metrics)
#' Metrics::rmse(mtcars.loocv.knn4$knn4.pred.mpg, mtcars$mpg)
#' plot(mtcars.loocv.knn4$knn4.pred.mpg, mtcars$mpg, main = "mpg predictions, LOOCV, KNN (k = 4)", xlab = "predicted", ylab = "actual")
#'
crossValidate <- function(df, codeString, numFolds, randomSort = TRUE){


  # Note for future versions:
  # should also add into returned data.frame:
  # new ID number (according to the reordered data), original order number,
  # and fold (group) number.
